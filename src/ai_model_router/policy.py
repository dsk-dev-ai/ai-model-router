...
def _sort_key(model: Model, policy: str, priority: list[str]) -> tuple[int, float]:
    if policy == "cheapest":
        return (0, model.price_in + model.price_out)
    if policy == "fastest":
        return (0, model.latency_hint)
    try:
        index = priority.index(model.id)
    except ValueError:
        index = len(priority) + 10_000
    return (index, model.latency_hint)


def rank_candidates(models: Iterable[Model], spec: RouterSpec) -> list[Model]:
    ...
    return sorted(candidates, key=lambda m: _sort_key(m, policy, wanted))
